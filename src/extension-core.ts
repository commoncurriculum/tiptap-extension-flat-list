    ...
    return {
      ...
      toggleFlatListItem:
        ...
        ({ state, commands }) => {
          ...
          return commands.setFlatListItem(listType, { ...attributesToCopy, ...attributes })
        },
      ...
      dedentFlatListItem:
        ...
        ({ state, tr, dispatch }) => {
          // Based on ProseMirror's setBlockType command.
          let applicable = false
          let lastDedented: { pos: number; oldIndent: number; node: ProseMirrorNode } | null = null
          for (const range of state.selection.ranges) {
            const {
              $from: { pos: from },
              $to: { pos: to },
          ...
          let subsequentItemPos = lastDedented!.pos + lastDedented!.node.nodeSize
          for (
...
function handleEnter(editor: Editor): boolean {
  ...
  if (!($to.parentOffset === 0 || $to.parentOffset === parentContentSize)) return false
...
function handleBackspace(editor: Editor): boolean {
  return editor.commands.undoInputRule() || handleBackspace1(editor) || handleBackspace2(editor)
}
