...
> We automatically setup a similar conversion for copied HTML, by setting a custom `editorProps.clipboardSerializer` when the editor is created. If you overwrite that, call `JoinListDOMSerializer.setClipboardSerializer(editor)` again afterwards to re-apply it.
