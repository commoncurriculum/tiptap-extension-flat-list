...
When serializing HTML for external consumption, it is good practice to convert flat list items to normal HTML lists: call `JoinListDOMSerializer.getHTML(editor)` in place of `editor.getHTML()`. It joins neighboring flat list items into a single `<ul>` and `<ol>` element, converts indented list items to nested HTML lists, and cleans up the HTML a bit.

> We automatically set up a similar conversion for copied HTML, by setting a custom `editorProps.clipboardSerializer` when the editor is created. If you overwrite that, call `JoinListDOMSerializer.setClipboardSerializer(editor)` again afterwards to re-apply it.
